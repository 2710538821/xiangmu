...
import { addTags, tagsDetail, updateTags } from '@/api/hmmm/tags.js'
export default {
  name: 'tagsAdd',
  props: {
    showDialog: {
  ...
  data () {
    return {
      ...
      formDataRules: {
        tagName: [{ required: true, message: '目录名称必填', trigger: 'blur' }]
      },
      subjectList: []
    }
  },
  ...
  methods: {
    // 获取目录详情
    async  getTagesDetail (id) {
      const { data } = await tagsDetail(id)
      this.formData = data
      // console.log(data)
    },
    async btnOk () {
      try {
        ...
        if (this.formData.id) {
          ...
          if (this.subjectName) {
            ...
            await addTags({ ...this.formData, subjectID: data[index].value })
          } else {
